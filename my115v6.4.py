...
class Uploader:
    ...
    def _cal_part_size(self, file_size: int) -> int:
        ...
        if self.config.MULTIPART_UPLOAD_MIN_SIZE <= file_size < self.config.SMALL_FILE_MAX_SIZE_FOR_5MB_CHUNKS:
            logging.debug(f"File size {format_bytes_to_human_readable(file_size)} is between {format_bytes_to_human_readable(self.config.MULTIPART_UPLOAD_MIN_SIZE)} and {format_bytes_to_human_readable(self.config.SMALL_FILE_MAX_SIZE_FOR_5MB_CHUNKS)}. Using custom part size: {format_bytes_to_human_readable(self.config.CUSTOM_CHUNK_SIZE_FOR_SMALL_FILES)}.")
            return self.config.CUSTOM_CHUNK_SIZE_FOR_SMALL_FILES
        elif file_size >= self.config.SMALL_FILE_MAX_SIZE_FOR_5MB_CHUNKS:
            logging.debug(f"File size {format_bytes_to_human_readable(file_size)} is >= {self.config.SMALL_FILE_MAX_SIZE_FOR_5MB_CHUNKS}. Using fixed part size: {format_bytes_to_human_readable(self.config.LARGE_FILE_FIXED_CHUNK_SIZE)}.")
            return self.config.LARGE_FILE_FIXED_CHUNK_SIZE
        else:
            logging.debug(f"File size {format_bytes_to_human_readable(file_size)} is less than multipart upload minimum ({format_bytes_to_human_readable(self.config.MULTIPART_UPLOAD_MIN_SIZE)}). Returning default part size (20MB).")
            return 20 * MB
...
class ApiService:
    ...
    def move_files(self, file_ids: List[str], to_cid: str, file_count: Union[int, None] = None) -> bool:
        ...
        if result and result.get("state"):
            logging.info(f"Successfully moved.")
            return True
        else:
...
def traverse_folder_bfs_concurrent(
    ...
    from collections import deque
    def _fetch_directory_items(cid: str, current_relative_path: str):
        items_list = []
        subdirs_list = []
        offset = 0
        page_size = config.API_FETCH_LIMIT
        while True:
            items, total = api_service.fetch_files_in_directory_page(
                cid=cid, limit=page_size, offset=offset,
                **config.COMMON_BROWSE_FETCH_PARAMS
            )
            if not items:
                break
            for item in items:
                ...
                full_rel = join_relative_path(current_relative_path, name)
                item_copy["_relative_path"] = full_rel
                items_list.append(item_copy)
                if is_item_folder(item):
                    sub_cid = _get_item_attribute(item, "fid", "file_id")
                    if sub_cid:
                        subdirs_list.append((sub_cid, full_rel))
            offset += len(items)
            if offset >= total:
                break
        return items_list, subdirs_list
    all_items = []
    queue = deque([(root_cid, root_name)])
    visited = set([root_cid])
    while queue:
        batch = []
        while queue and len(batch) < config.API_CONCURRENT_THREADS:
        ...
        results = api_service._fetch_concurrent_pages(_fetch_directory_items, fetch_args_list)
...
class FolderOperationService:
    ...
    def execute_page2(self, current_page_items: List[Dict]) -> str:
        ...
        if not all_items:
            ...
            return CMD_CONTINUE_INPUT
        original_name = self._get_original_folder_name_from_paths(browser.state.current_folder_id)
    ...
    def _get_browser(self):
        # 临时 hack：通过 state 反查 browser
        import builtins
        if hasattr(builtins, '_current_browser'):
            return builtins._current_browser
        raise RuntimeError("Browser instance not available")
...
class CommandHandler:
    def __init__(self, browser: 'FileBrowser'):
        ...
        self.folder_op_service = FolderOperationService(self.api_service, self.config, self.state)
...
class CommandProcessor:
    def __init__(self, command_handler: CommandHandler):
        ...
        self.prefix_command_map = {
            ...
            'jump': self.handler.handle_jump,  # ← 新增 jump
        }
...
class FileBrowser:
    ...
    def _fetch_all_items_and_update_state(self):
        logging.debug(f"Fetching all items for: {self.state.title}")
    ...
    def recursively_download_folder(self, folder_info: Dict, current_download_path: str, prefix_item_name: str = "Current Task"):
        ...
        def item_handler(item):
            ...
            logging.debug(f"Collected file for download: {file_relative_path}")
        def folder_handler(folder):
            ...
            logging.debug(f"Ensured directory exists: {full_dir_path}")
    ...
    def rs(self, action_choice: str, page_items: List[Dict]) -> str:
        ...
        all_items = []
        for fid, name in root_folders:
            items = self._concurrent_traverse_folder(fid, name)
            all_items.extend(items)
        folder_items = [item for item in all_items if is_item_folder(item)]
        if not folder_items:
            logging.info(f"No subfolders found.")
            return CMD_CONTINUE_INPUT
        fids = extract_fids(folder_items)
        if not fids:
            logging.warning("No valid folder IDs found.")
            return CMD_CONTINUE_INPUT
        logging.info(f"Fetching details for {len(fids)} folders...")
        details_map = self.api_service.get_items_details_batch(fids)
        final_results = []
        for item in folder_items:
            ...
            if size_bytes <= threshold_bytes:
                full_path = item.get("_relative_path", f"unknown_path_{fid}")
    ...
    def merge(self) -> str:
        ...
        for idx, src_folder in enumerate(source_folders, 1):
            ...
            # Step 1: 每次都重新 fetch 当前目录已有项（动态感知变化）
            logging.debug("  → Refreshing target directory contents for conflict detection...")
            ...
            # Step 2: 获取源文件夹的直接子项（第一层）
            logging.debug(f"  → Fetching direct children of '{src_name}'...")
            sub_items = []
            offset = 0
            while True:
                ...
                if not sub_is_folder:
                    ...
                    if sub_name_lower in existing_target_items:
                        ...
                        if existing_is_folder:
                            # ✅ 同名且是 folder → 注入：把 sub_item 的所有直接子项移入 existing_fid
                            logging.debug(f"    ↳ Folder '{sub_name}' exists → injecting contents into {existing_fid}")
                            gc_offset = 0
                            while True:
                            ...
                            # ❌ 同名但不是 folder（如文件）→ fallback：整体移动该子文件夹到顶层
                            logging.debug(f"    ↳ Name conflict: '{sub_name}' exists as file → moving folder as-is")
                            move_plan.append((sub_fid, target_cid))
                    else:
                        # 无冲突 → 整体移动子文件夹到 target_cid
                        logging.debug(f"    ↳ No conflict → moving folder '{sub_name}' as-is")
                        move_plan.append((sub_fid, target_cid))
            ...
            # Step 4: 执行本 folder 的 move（仍支持分批）
            ...
            # Step 5: 记录 & 汇总
            #_log_move_operation([src_fid], target_cid, self.config)  # 记录“源头” folder
            if folder_fail == 0:
            ...
            # 可选：清空当前 cache，强制下次刷新
            self.state._last_fetched_params_hash = None

        # ——— 最终汇总 ———
        summary = f"\n✨ Merge Summary: {len(source_folders)} folders processed"
        if total_success:
