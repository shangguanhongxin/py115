...
# ANSI Color Codes
COLOR_FOLDER = '\033[0m'      # Bright Blue
COLOR_FILE = '\033[0m'         # Default (or white)
COLOR_SIZE_SMALL = '\033[92m'  # Green
COLOR_SIZE_MEDIUM = '\033[93m' # Yellow
COLOR_SIZE_LARGE = '\033[91m'  # Red
COLOR_RESET = '\033[0m'

'''
需要rclone，mpv
全局变量中token远程路径、快捷列表必须自定义，其他变量可以自定义
全局变量中infuse需要和已安装的infuse ua一致，请自行检查是否一致,当前为mac最新版本 20250628
安卓termux下调用的是mpv-ytdl版本，如果用的是其他版本如mpv-android，mpvkt，reex，请自行修改，配置文件中记得加上infuse的ua
'''
...
class AppConfig:
    """
    Encapsulates global configuration and mutable state for the application.
    """
    def __init__(self):
        # API URLs
        ...
        self.CUSTOM_CHUNK_SIZE_FOR_SMALL_FILES = 20*1024*1024 # 15MB 上面两个大小之间的文件的上传切片
        self.LARGE_FILE_FIXED_CHUNK_SIZE = 20*1024*1024 # 100MB 大于50MB的文件的上传切片大小
            ...
            if is_item_folder(item):
                ...
                try:                    
                    ...
                    if size_bytes is not None:
                        ...
                        colored_name = name_value  # 无大小信息，不着色
                except (ValueError, TypeError):
                    colored_name = name_value  # 解析失败，不着色

            # 主行：[0] Name: xxx（无 [File]/[Folder]）
            main_line = f"[{idx_padded}] {colored_name}"
            logging.info(main_line)

            # 显示详情（无颜色，保持原样）
            if display_full_details:
                indent_len = len(f"[{idx_padded}] ") + len(name_value)
                ...
                if row_data.get("id_value"):
                    detail_lines.append(f"ID: {row_data['id_value'].ljust(max_id_display_len)}")
                if row_data.get("pick_code_value"):
                    detail_lines.append(f"Pick Code: {row_data['pick_code_value'].ljust(max_pick_code_display_len)}")
                '''
...
class FileBrowser:
    ...
    def _command_c(self, action_choice: str, page_items: List[Dict]) -> str:
        ...
        if len(selected_indices) != 1:
            ...
            return CMD_CONTINUE_INPUT
        item_info = page_items[selected_indices[0]]
